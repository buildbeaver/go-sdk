...
import (
	...
	"github.com/controlci/sdk/dynamic/bb"
)
...
func main() {
	bb.Workflows(
		...
		bb.NewWorkflow().
			Name("generate").
			Handler(submitGenerateJobs).
			Depends("base", bb.WorkflowTerminateOnFailure),
	...
	w.Job(bb.NewJob().
		...
		OnSuccess(func(event *bb.JobStatusChangedEvent) {
			...
			goDockerConfig := bb.NewDocker().
				Image(fmt.Sprintf("733436759586.dkr.ecr.us-west-2.amazonaws.com/go-builder:%s", *jGraph.Job.Fingerprint)).
				Pull(bb.DockerPullIfNotExists).
				Shell("/bin/bash").
...
func submitUnitTestJobs(w *bb.Workflow) error {
	...
	w.Job(bb.NewJob().
		...
		Env(bb.NewEnv().
			...
			Value("postgres://bb-user:not-a-real-password@postgres:5432/?sslmode=disable")).
		Step(bb.NewStep().
